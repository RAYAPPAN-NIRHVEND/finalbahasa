...
app.use(cors({ origin: '*', methods: ['GET','POST','PUT','DELETE','OPTIONS'], allowedHeaders: ['Content-Type','Authorization','x-admin-password'] }));
app.options('*', cors());
...
app.post('/api/payment/submit', auth, async (req, res) => {
    try {
        // Terima JSON - proof opsional (konfirmasi via WhatsApp)
        const { packageType, package: pkgName, packageName, method, points, amount } = req.body;

        // Jika pakai packageType (lama), validasi paket
        let pkg;
        if (packageType) {
            ...
            pkg = validPkgs[packageType];
            if (!pkg) return res.status(400).json({ error: 'Paket tidak valid' });
        } else {
            // Terima data paket bebas dari frontend
            pkg = { points: parseInt(points)||0, price: parseInt(amount)||0, name: packageName || pkgName || 'Custom' };
        }
        ...
        await sendMail(process.env.ADMIN_EMAIL,'💳 Pembayaran Baru - PolyglotQuest',`<div style="font-family:Arial;padding:20px;"><h2 style="color:#f59e0b;">💳 Pembayaran Baru</h2><p><b>User:</b> ${user.name} (${user.email})</p><p><b>Paket:</b> ${pkg.name}</p><p><b>Total:</b> Rp ${pkg.price.toLocaleString('id-ID')}</p><p><b>Metode:</b> ${method||'-'}</p><p style="color:#f59e0b;">User akan konfirmasi via WhatsApp</p><a href="${ADMIN_URL}" style="background:#6366f1;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:12px;">Buka Admin Panel</a></div>`);
        ...
        res.json({ success: true, message: 'Pembayaran disubmit, menunggu verifikasi admin.', payment, id: payment.id });
    } catch(e) { console.error(e); res.status(500).json({ error: 'Server error: ' + e.message }); }
});
